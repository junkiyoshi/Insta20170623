...
void ofApp::draw() {
	...
	for (int i = 0; i < triangles.size(); i++) {
		...
		for (int j = 0; j < 3; j++) {
			noise_value = ofNoise(triangles[i].getVertex(j).x * 0.005, triangles[i].getVertex(j).y * 0.005, triangles[i].getVertex(j).z * 0.005, ofGetFrameNum() * 0.01);
			...
			if (noise_value < 0.6) {
				...
				c = ofColor(255);
				tmp_triangles[i].setVertex(j, triangles[i].getVertex(j).normalized() * 250 + triangles[i].getVertex(j).normalized() * noise_value * 50);
			}
